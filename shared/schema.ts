...
// Client-only schemas for the new leasing flow (no database tables)
// Note: Percentage fields (interestRate, residualValueRate) are stored as percentages (0-100), not decimals
export const leaseInputSchema = z.object({
  assetCost: z.number().min(1000, "Asset cost must be at least $1,000"),
  downPayment: z.number().min(0, "Down payment cannot be negative"),
  financedAmount: z.number().min(0, "Financed amount cannot be negative"),
  termMonths: z.number().min(6, "Term must be at least 6 months").max(120, "Term cannot exceed 120 months"),
  interestRate: z.number().min(0.1, "Interest rate must be at least 0.1%").max(50, "Interest rate cannot exceed 50%"),
  monthlyAdminFee: z.number().min(0, "Monthly admin fee cannot be negative"),
  residualValueRate: z.number().min(0, "Residual value rate cannot be negative").max(100, "Residual value rate cannot exceed 100%"),
  startDate: z.coerce.date(),
}).superRefine((data, ctx) => {
  // Cross-field validation for monetary consistency
  if (data.downPayment > data.assetCost) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Down payment cannot exceed asset cost",
      path: ["downPayment"],
    });
  }

  const expectedFinanced = data.assetCost - data.downPayment;
  const tolerance = 0.01; // Allow small floating point differences
  if (Math.abs(data.financedAmount - expectedFinanced) > tolerance) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: `Financed amount should equal asset cost minus down payment (${expectedFinanced.toFixed(2)})`,
      path: ["financedAmount"],
    });
  }

  if (data.financedAmount < 1000) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Financed amount must be at least $1,000",
      path: ["financedAmount"],
    });
  }
});
